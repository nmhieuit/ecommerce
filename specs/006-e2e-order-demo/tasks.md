...
- [X] T032 [US3] Capture per-step stills in `frontend/apps/web/demo/order-demo.spec.ts` — `docs/demo/01-catalog.png`, `02-basket.png`, `03-checkout.png`, `04-confirmation.png` — writing to `docs/demo/` rather than Playwright's default output, which is git-ignored (FR-013a, research Decision 8)
- [X] T033 [US3] Add the hop-evidence step to both scripts: mark a timestamp before the flow, then read `docker compose logs --since <timestamp> otel-collector` into `artifacts/demo/hops.txt`. **Parse `ResourceTraces` blocks specifically** — bare `service.name=` also matches `ResourceLog` lines from the logs pipeline — and **drop every span whose `url.path` starts with `/health`**, or Docker's five-second healthchecks make all six services look busy on a stack where nothing ran (research.md, follow-up measurement T005)
- [X] T034 [US3] Assert in both scripts that `Gateway.Api`, `Bff.Api`, `Products.Api`, `Baskets.Api`, and `Orders.Api` each recorded at least one **non-health** span in the window, failing the run if any is missing. Report counts but do not assert fixed numbers. The filter is what gives this assertion meaning — unfiltered, it passes on a stack where the demo never ran (FR-011a, SC-006a)
- [X] T035 [US3] Write `docs/demo-phase-1.md`: prerequisites, the ordered procedure with its starting state (FR-007b, FR-010), each hop on the checkout path named in order (FR-011), the embedded stills, where the video lives (SCRUM-16), and a table mapping every Phase 1 exit criterion to *evidenced by this run* or *deferred to Phase N* (FR-015). Fold in T017's cold-start result
- [X] T036 [US3] Link the demo command and the walkthrough from `docs/local-development.md`'s command table, so both are reachable from the entry point a contributor already uses (FR-014, SC-005)
- [X] T037 [US3] Confirm the video lands in `artifacts/demo/video/` and nowhere else: run `git status --porcelain docs/` and `git check-ignore artifacts` per quickstart Scenario 7 (FR-013)

**Checkpoint**: a reviewer who was not present can follow the flow from committed files alone. **Verified** on 2026-08-19: `docs/demo-phase-1.md` plus four stills, 84 KB committed in total, every link resolving; `artifacts/` confirmed git-ignored and no `.webm` anywhere committable.

**The hop-evidence filter earned its place, measured.** In one demo window `Parties.Api` emitted 461 spans and **all 461 were health checks** - Docker probes every service every five seconds. Counting raw spans, a component that served nothing looks busy, and the FR-011a assertion would pass on a stack where the demo never ran. With the filter, Parties correctly does not appear and the five real hops do. The assertion was also tested against a deliberately truncated evidence file to confirm it fails when a component is missing, rather than only being observed to pass.

**One still was a lie and was removed.** The first version captured `03-checkout.png` after focusing the checkout button; it came out **byte-identical** to `02-basket.png`, because the focus ring does not survive into a screenshot. It was replaced by `04-basket-empty.png`, which shows the basket emptied after checkout - visually distinct, and evidence for FR-017. Four stills, all distinct.

---

## Phase 6: Polish & Cross-Cutting Concerns

- [X] T038 [P] Run every scenario in `specs/006-e2e-order-demo/quickstart.md` end to end and record which requirement each cleared. **DONE**: Scenario 1 (demo runs, exit 0), 2 (persisted total 59.25 matches the confirmation), 3 (tenantId `contoso`), 4 (both read and write refuse without a tenant - 500, no row created), 5 (five consecutive runs, five distinct orders), 6 (five components in `hops.txt`, Parties correctly absent), 7 (walkthrough and four stills committable, `artifacts/` ignored, no `.webm` under `docs/`), 8 (cold start 2m48s), 9 (empty-basket checkout 409 with an actionable message and no order; downstream-unavailable covered by T039)
- [X] T039 [P] Run quickstart Scenario 9's downstream-unavailable check (`docker compose stop orders-api`, demo, restart) and confirm the failure names what was unreachable and leaves no partial order (FR-016, SC-008). **DONE - and it found a wrong diagnosis.** With orders-api stopped the demo failed correctly (exit 1) but reported "the stack is not running in demo mode", which is false and points at the wrong fix. Three faults look identical from outside - never published, died, or up-and-unwell - so the check now asks Compose and Docker which it is. Verified: a stopped service reports "the orders service belongs to this stack but is not running"; a default-mode stack reports "up but not in demo mode - its port is not published". Empty-basket checkout returns 409 with "There is nothing in your basket to check out." and creates no order
- [X] T040 [P] Run the full build and test suite, confirming nothing in this feature broke the structure or container conventions. **DONE**: `dotnet build Ecommerce.slnx` exit 0 **with warnings as errors**; `dotnet test Ecommerce.slnx` exit 0 - **16 test projects, 247 tests, zero failures**, including the structure and container convention suites (9 + 9) and `CrossServiceIsolation.Tests` (14). Frontend: 46 Vitest tests, `tsc --noEmit` clean, ESLint clean across the workspace
- [X] T041 Measure the repeat-run time against the plan's 90-second budget and record the actual figure in `docs/demo-phase-1.md`. **DONE**: a `--skip-start` run takes **10 seconds** against a 90-second budget. Recorded in the walkthrough beside the cold-start figures
- [ ] T042 Attach the video recording to SCRUM-16 and confirm the walkthrough's pointer to it resolves. Mark the story's acceptance criteria against the evidence produced. **BLOCKED - needs a person.** Two reasons: (1) the Atlassian tooling available covers comments, edits, and transitions but **not file attachments**, so the `.webm` must be uploaded by hand from `artifacts/demo/playwright/**/video.webm`; (2) posting to Jira is outward-facing and visible to the team, so it is not something to do unprompted. Everything such a comment would say is already assembled in `docs/demo-phase-1.md`
