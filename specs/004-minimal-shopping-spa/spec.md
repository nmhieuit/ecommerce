...
- **FR-014**: The storefront MUST send every backend request to the single backend surface (the gateway/BFF) and MUST NOT call any individual service directly. *(Amended by spec 014 follow-up: the sign-in form's one call to the identity server's token endpoint, made only to obtain a token, is the sole exception — see FR-015.)*
- **FR-015**: Every request the storefront makes MUST carry the tenant context resolved at the edge, and no screen may require the shopper to supply, choose, or see a tenant or user identity. *(Amended: spec 014 replaced the Phase 1 stub with real JWT bearer authentication at the gateway, so the storefront now signs the shopper in and proves who they are on every request. The tenant is still resolved at the edge from the token's `tenant_id` claim — the shopper never supplies or chooses one.)*
- **FR-026** *(added with the identity cutover)*: Every storefront screen except sign-in MUST require a signed-in shopper. A signed-out visitor MUST be sent to a sign-in screen where they enter a username and password; a wrong credential MUST produce a clear message distinct from an identity-server outage; a successful sign-in MUST return them to where they were headed and every later request MUST carry `Authorization: Bearer <token>`. When the gateway refuses the token (expired or revoked) or the shopper chooses Sign out, the session MUST end, cached shopper data MUST be discarded, and the shopper MUST be sent back to sign-in.
...
- ~~Phase 1 has exactly one shopper (the existing stub identity) and one tenant. There is no sign-in, sign-up, account, or tenant selection screen, and none is in scope.~~ **Superseded by the identity cutover (spec 014):** shoppers are real identity-server users, each with one tenant, and the storefront has a sign-in screen (FR-026). Sign-up, account management, and tenant selection remain out of scope.
- The sign-in form uses the Resource Owner Password grant on the dedicated public client `ecommerce-web-spa-password`, because Authorization Code + PKCE (the SPA's primary client) needs an interactive login page the identity server does not have yet. That client is opt-in per environment (`SpaPasswordClient:Enabled`), enabled in the local stack only, and must not be enabled in production. Replace it with Authorization Code + PKCE when the login UI lands.
