...
**AMENDED trong lúc triển khai (tasks.md T025)**: Toggle KHÔNG dùng Unleash thật — đọc từ configuration (`FeatureToggles:IdentityServerAuthCutover`, `services/gateway/src/Gateway.Api/Identity/FeatureToggleOptions.cs`) qua `IOptionsMonitor` + `AddPolicyScheme.ForwardDefaultSelector` (đánh giá lại mỗi request từ `context.RequestServices`, không phải một lần lúc khởi động — nên hot-reload một ConfigMap vẫn có tác dụng mà không cần restart pod). Lý do: ADR-0008 đã CHỌN Unleash nhưng CHƯA CÓ service nào trong nền tảng thực sự triển khai nó — cả 3 Action Item của ADR-0008 (deploy self-hosted Unleash, tích hợp SDK .NET/React, xây CI check hạn dùng toggle) vẫn chưa được đánh dấu hoàn thành ở bất kỳ đâu trong repo. Xây dựng toàn bộ hạ tầng Unleash (server + Postgres + SDK wiring toàn nền tảng) chỉ để phục vụ một toggle duy nhất của tính năng này là vượt phạm vi hợp lý. Thiết kế đọc qua `IOptionsMonitor` cố ý để việc thay nguồn đọc bằng `IFeatureManager` do Unleash cấp sau này chỉ là một thay đổi một dòng tại điểm đọc, không phải thiết kế lại cơ chế toggle-gating.
