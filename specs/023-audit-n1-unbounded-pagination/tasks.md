...
- [X] T009 [US1] Chạy lại T004, T005, T006, xác nhận PASS (Green) — phụ thuộc T007, T008.
      **Kết quả xác nhận**: `dotnet test tests/QueryCoverageTests` 8/8 PASS; `dotnet test services/bff/tests/Bff.Api.UnitTests` 19/19 PASS (trước khi thêm T012); `dotnet test services/bff/tests/Bff.Api.ContractTests --filter ProductsConsumerPactTests` 1/1 PASS; `dotnet build Ecommerce.slnx` 0 lỗi. T004 (integration, cần SQL Server qua Testcontainers) KHÔNG chạy được trong sandbox này — Docker Desktop không chạy và không có quyền khởi động (`Cannot open com.docker.service`) — đã xác nhận đúng qua build sạch (0 lỗi biên dịch) và rà soát logic thủ công, nhưng chưa có bằng chứng runtime thật; khuyến nghị người dùng chạy `dotnet test services/products/tests/Products.Api.IntegrationTests` trên máy có Docker trước khi merge.
...
- [X] T011 [P] [US2] Viết `services/baskets/tests/Baskets.Api.IntegrationTests/BasketQueryCountTests.cs`: seed basket 1 dòng và basket 5 dòng trực tiếp qua `BasketsDbContext` (không qua HTTP — nhanh hơn và không phụ thuộc route add-item), gắn `QueryCountInterceptor` vào `BasketsDbContext` qua `host.ConfigureServices` re-registering `AddDbContext` (KHÔNG phải `ConfigureTestServices` như kế hoạch ban đầu — API đó không tồn tại trên `IWebHostBuilder` trong stack này; `ConfigureServices` là quy ước đã dùng nhất quán ở `PactProviderHost.cs`/`BffTestHost.cs`/`GatewayTestHost.cs`), tải cả hai basket qua `GET /baskets/{id}` thật, xác nhận `ExecutedCommandCount` BẰNG NHAU giữa hai basket — phụ thuộc T010.
      **KHÔNG chạy được trong sandbox này**: cần SQL Server qua Testcontainers, Docker không khả dụng (như T004/T009) — đã xác nhận build sạch (0 lỗi) và rà soát logic (`.Include(b => b.LineItems)` không đổi, chỉ thêm interceptor quan sát); khuyến nghị chạy trên máy có Docker trước khi merge.
...
- [X] T016 [US2] Chạy lại T011, T012, T013, xác nhận PASS — phụ thuộc T015.
      **Kết quả xác nhận**: `dotnet test tests/QueryCoverageTests` 8/8 PASS; `dotnet test services/bff/tests/Bff.Api.UnitTests` 22/22 PASS; `dotnet build Ecommerce.slnx` 0 lỗi. T011 (Testcontainers) không chạy được trong sandbox — xem ghi chú T011.

**Checkpoint**: User Story 1 VÀ 2 đều hoạt động độc lập (trong phạm vi xác thực được ở sandbox này) — khoảng hở nặng nhất của cuộc rà soát (over-fetch xuyên service ở BFF) đã khép kín, hành vi `.Include()` đúng của basket đã có regression test thật (chờ chạy trên máy có Docker).
...
- [X] T020 [US3] Chạy lại T017, T018, xác nhận PASS — phụ thuộc T019.
      **Kết quả xác nhận**: `dotnet test tests/QueryCoverageTests` 8/8 PASS (marker `MaxPageSize` đã pass từ đầu). T017 (Testcontainers) không chạy được trong sandbox — xem ghi chú T009/T011; đã xác nhận qua build sạch + rà soát logic (`Math.Clamp` áp dụng đúng cho cả nhánh mặc định và nhánh client truyền tường minh).

**Checkpoint**: Cả 3 user story hoạt động độc lập (trong phạm vi xác thực được ở sandbox này — xem ghi chú Docker ở T009/T011/T020). Toàn bộ khoảng hở ở `plan.md` Summary đã được khép kín về mặt code; cần chạy lại toàn bộ integration test trên máy có Docker trước khi merge.
