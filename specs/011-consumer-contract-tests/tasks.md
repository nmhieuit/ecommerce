...
- [ ] T001 Add a `PactNet` `5.0.1` `PackageVersion` entry to `Directory.Packages.props`, next to the
      `xunit`/testing entries, with a comment referencing ADR-0006 and research.md Decision 1
- [ ] T002 [P] Create `services/bff/tests/Bff.Api.ContractTests/Bff.Api.ContractTests.csproj`
...
- [ ] T005 [P] Create `services/orders/tests/Orders.Api.ContractTests/Orders.Api.ContractTests.csproj`
      (same package set as T003 plus `Microsoft.AspNetCore.Mvc.Testing`; `ProjectReference` to
      `services/orders/src/Orders.Api/Orders.Api.csproj` **and** to
      `shared/EventContracts/EventContracts.csproj` for `BasketCheckedOutV1`, needed by the event
      pilot's consumer test in Phase 4)
- [ ] T006 [P] Create `pacts/README.md` documenting the four-boundary table from
...
- [ ] T007 Add the four new projects (T002–T005) to `Ecommerce.slnx`
- [ ] T008 Run `dotnet build Ecommerce.slnx` and confirm the solution — including the four new,
...
- [ ] T009 [P] [US1] Write `ProductsConsumerPactTests.cs` in
...
- [ ] T010 [P] [US1] Write `BasketsConsumerPactTests.cs` in
...
- [ ] T011 [P] [US1] Write `OrdersConsumerPactTests.cs` in
...
- [ ] T012 [US1] Write `ProductsProviderPactTests.cs` in
...
- [ ] T013 [US1] Revert-and-confirm-red for the products boundary: temporarily rename `Price` to
...
- [ ] T014 [US1] Write `BasketsProviderPactTests.cs` in
...
- [ ] T015 [US1] Revert-and-confirm-red for the baskets boundary: temporarily rename `Total` to
      ...
      green again
- [ ] T016 [US1] Write `OrdersProviderPactTests.cs` in
...
- [ ] T017 [US1] Revert-and-confirm-red for the orders boundary: temporarily rename `PlacedAtUtc` to
...
- [ ] T018 [US2] Write `BasketCheckedOutConsumerPactTests.cs` in
...
- [ ] T020 [US2] Write `BasketCheckedOutProviderPactTests.cs` in
...
- [ ] T021 [US2] Revert-and-confirm-red for the event boundary: temporarily swap the `Total` argument
...
- [ ] T022 [US3] Create `tests/ContractCoverageTests/ContractCoverageTests.csproj` (net10.0,
...
- [ ] T023 [US3] Implement `tests/ContractCoverageTests/ContractCoverageScanner.cs`, modeled on
...
- [ ] T024 [US3] Implement `tests/ContractCoverageTests/ContractCoverageTests.cs`, modeled on
      ...
      T009–T021)
- [ ] T025 [US3] Revert-and-confirm-red for coverage: temporarily rename
...
- [ ] T026 [P] Run `dotnet build Ecommerce.slnx` and confirm the whole solution — the five new test
      ...
      cleanly
- [ ] T027 [P] Run `git status` and `git diff` at the repo root and confirm the only changes are
...
- [ ] T028 Walk through [quickstart.md](./quickstart.md) SC-001 through SC-004 top to bottom exactly
