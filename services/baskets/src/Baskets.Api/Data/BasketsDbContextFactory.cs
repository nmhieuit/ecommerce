...
/// <summary>
/// Builds a <see cref="BasketsDbContext"/> for the <c>dotnet ef</c> tooling only.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class BasketsDbContextFactory : IDesignTimeDbContextFactory<BasketsDbContext>
{
    public BasketsDbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<BasketsDbContext>()
            .UseSqlServer(configuration.GetConnectionString("BasketsDb"))
            .Options;

        return new BasketsDbContext(options);
    }
}
