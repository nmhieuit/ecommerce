...
public class TenantEnforcementTests(SqlServerFixture sqlServer) : IClassFixture<SqlServerFixture>
{
    ...
    /// <summary>
    /// Kiểm tra: lấy `BasketsDbContext` từ DI khi chưa có tenant nào được phân giải thì ném
    ...
    /// được database.
    /// Task nguồn: spec 003 (danh tính giả lập và tenant) — T025, US2.
    /// </summary>
    [Fact]
    public async Task ResolvingTheDbContext_Throws_WhenNoTenantHasBeenResolved()
    {
        ...
        // không ném hoặc ném loại khác. Đạt khi việc tạo DbContext bị chặn vì chưa biết tenant.
        Assert.Throws<MissingTenantContextException>(
            () => scope.ServiceProvider.GetRequiredService<BasketsDbContext>());
    }

    /// <summary>
    /// Kiểm tra: gọi thẳng `/baskets/{id}` mà không có `X-Tenant-Id` trả về `500 Internal Server
    /// Error`.
