...
{
    private static readonly string PartyRoute = $"/parties/{Guid.NewGuid():D}";

    /// <summary>
    /// Kiểm tra: gọi thẳng `Parties.Api` (không qua gateway/BFF) mà không kèm token nào bị chính
    ...
    public async Task ARequestWithNoToken_IsRejected()
    {
        ...
        var response = await client.GetAsync(PartyRoute);
    ...
    /// <summary>
    /// Kiểm tra: token hợp lệ bị nối thêm chuỗi rác (làm hỏng chữ ký), gửi thẳng tới `Parties.Api`
    ...
    public async Task ARequestWithATamperedToken_IsRejected()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, PartyRoute);
