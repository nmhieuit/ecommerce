...
describe('Confirmation', () => {
   ...
   * Lý do: FR-009/SC-005: so mã người mua thấy với đơn trong backend; mã bị rút gọn không còn duy
   * nhất một cách đáng tin.
  ...
  it('shows the order identifier verbatim', () => {
    ...
    // toBeInTheDocument(): xanh khi phần tử có trong DOM. Mã đơn hiện nguyên văn để người mua tra
    // cứu; đỏ khi bị cắt/định dạng.
    expect(screen.getByText(order.id)).toBeInTheDocument();
  });

  /**
   * Kiểm tra: hiển thị tổng đơn bằng USD với 2 chữ số thập phân.
   * Lý do: FR-009 và FR-024: tổng tiền trên màn hình xác nhận phải là số tiền người mua đọc được.
   * Task nguồn: spec 004 (SPA mua sắm tối thiểu) — T053, US3 (FR-009, FR-024).
   */
  it('shows the order total in the single Phase 1 currency', () => {
    renderInRouter(<Confirmation order={order} />);

    // toBeInTheDocument(): xanh khi phần tử có trong DOM. Tổng hiện theo đô la, 2 số thập phân.
    expect(screen.getByText('$59.25')).toBeInTheDocument();
  });

  /**
   * Kiểm tra: màn hình nói rõ đơn đã được đặt.
   * Lý do: người mua cần lời xác nhận rõ ràng ngoài con số, đúng tên gọi "màn hình xác nhận" của
   * US3.
  ...
  it('tells the shopper their order was placed', () => {
    ...
    // toBeInTheDocument(): xanh khi phần tử có trong DOM. Có tiêu đề xác nhận (chứa 1 trong 3 cụm);
    // đỏ khi không có lời xác nhận.
   ...
   * hiển thị".
   * Lý do: Edge Cases của spec: không được hiện màn hình hỏng hay bịa ra 1 mã đơn — tuyệt đối không
   * có mã tự chế.
   * Task nguồn: spec 004 (SPA mua sắm tối thiểu) — T053, US3 (FR-009).
   */
  it('shows a nothing-to-show state when there is no order', () => {
    ...
    // toBeInTheDocument(): xanh khi phần tử có trong DOM. Hiện thông báo không có đơn gần đây.
    ...
    // thấy). Không hiện mã đơn cũ (queryBy trả null, .not đảo điều kiện).
