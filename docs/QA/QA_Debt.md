...
- **Lượt làm lại theo hướng Postman (2026-09-26) — hợp đồng còn tác dụng trên hệ thống chạy thật**: folder Postman `11 - Hợp đồng BFF ↔ service (Pact)` gọi thẳng products/baskets/orders và kiểm đúng những gì `pacts/bff-*.json` ghi (mã trạng thái, kiểu từng trường — price/total/unitPrice phải là số): baseline 10/10 xanh. Đổi `ProductResponse(Guid Id, string Name, decimal Price)` thành `…decimal UnitPrice)`, dựng lại image `products-api`: Postman bước 01 **đỏ** (thiếu `price`), provider test `CatalogResponses_SatisfyTheBffsRecordedExpectations` đỏ với `$.items[0] -> Actual map is missing the following keys: price`; hoàn tác + dựng lại → Postman 10/10 xanh lại. Gỡ `ProductsProviderPactTests.cs` (đổi tên `.bak`) → `ContractCoverageTests` đỏ nêu `Boundary = BFF-products`; khôi phục → 6/6 xanh.
- **1 lần đỏ chưa giải thích ở provider products**: trong lượt chạy liên tiếp (consumer BFF → consumer event → `git checkout -- pacts` → provider products/baskets/orders), `Products.Api.ContractTests` đỏ 1/1 ở lần đầu (19 giây, không thấy thông điệp) trong khi baskets/orders xanh; chạy lại đúng chuỗi đó và 3 lần riêng lẻ đều xanh (18–24 giây). Không tái hiện nên không kết luận được; ghi nhận thêm vào rủi ro chập chờn đã nêu ở trên (xung đột cổng/Testcontainers trên Windows).
- **Mỗi lần chạy consumer BFF viết lại cả 3 file pact**: sau `dotnet test services/bff/tests/Bff.Api.ContractTests`, `git status` báo `M` cho `pacts/bff-baskets.json`, `bff-orders.json`, `bff-products.json` (7 dòng — JWT giả ký lại mỗi lần chạy); phải `git checkout -- pacts` nếu không muốn đưa nhiễu vào commit. README của `pacts/` nói "regenerate deliberately" nhưng chưa nói tới việc chỉ số dòng token đổi mỗi lần.
- **Hợp đồng sự kiện `BasketCheckedOut` không kiểm thủ công được**: chưa có publisher/consumer thật (chỉ `BasketCheckedOutMapper` dựng payload cho provider test) — xem mục 008.

## 012 — Cổng chất lượng SonarQube (hạ tầng nền của 013)

*Ghi chú phạm vi: spec 012 chưa từng có doc QA riêng vì không có architecture/development/PO cho nó (thư mục `specs/012-sonarqube-quality-gate` đã bị xoá); lượt này QA dựa trên `docs/spec-summary-vi/012-*.json` + mã CI + SonarQube/Jenkins cục bộ dựng lại từ volume cũ. Phần GitHub branch protection nằm ở mục 013.*

- **[NGHIÊM TRỌNG] `Jenkinsfile` đang commit `CI_FAST_ITERATION = 'true'` — 4 chặng bị bỏ qua và không có check thay thế nào được đăng (đã xác minh bằng mã + lịch sử Jenkins thật)**: khối `environment` (dòng 93) đặt `'true'` kèm chú thích "TEMP … MUST flip back to 'false' and run one full real pipeline before Phase 3 is considered done" (banner đầu file, ngày 2026-08-27); các chặng `sonarqube: begin analysis`, `integration tests`, `contract tests`, `deployment manifest lint` (019) và `sonarqube quality gate` đều có `when { environment name: 'CI_FAST_ITERATION', value: 'false' }` nên bị bỏ qua khi cờ là `'true'`. Banner nói "the five required check names still get published on every run — only the work behind three of them is skipped", nhưng trong file **không có** stage stub nào gọi `checkPassed`/`githubNotify` cho 3 tên đó (grep `stub|FAST` chỉ ra banner + các `when`) — nên `ci/integration-tests`, `ci/contract-tests`, `ci/sonarqube-quality-gate` không bao giờ được đăng. Lịch sử Jenkins trong volume `ecomerce-ci_jenkins-data` khớp: log các build 2026-09-02 → 09-06 có `Stage "sonarqube: begin analysis" skipped due to when conditional`. Hệ quả: (a) nếu branch protection được dựng lại với 5 required checks, mọi PR sẽ chờ vô hạn 3 check không bao giờ tới (chặn oan); (b) nếu không, cổng chất lượng không đo gì. Cần chủ sở hữu đặt `'false'` (hoặc tham số hoá) và chạy 1 pipeline đầy đủ như chính banner yêu cầu.
- **Phân tích SonarQube của `master` đã cũ ~25 ngày**: SonarQube cục bộ (`docker compose -f docker-compose.ci.yml up -d --no-build sonarqube`, dữ liệu volume cũ) cho `project_branches/list` → `master` phân tích lần cuối **2026-09-01**, PR mới nhất được phân tích cũng 2026-09-01 (10 PR, cả 10 `OK`); Jenkins build cuối 2026-09-06. Khớp phát hiện ở mục 013 (CI ngừng). Số đo lúc đó: coverage 78.8 %, trùng lặp 7.9 %, 47 code smell, 6 bug, 1 lỗ hổng, 3342 dòng — chỉ phản ánh mã đến đầu tháng 9 (nhiều spec 015–026 chưa từng được phân tích).
- **Gate là "Sonar way" built-in, không chỉnh được**: `qualitygates/show` → `new_violations > 0`, `new_coverage < 80`, `new_duplicated_lines_density > 3`, `new_security_hotspots_reviewed < 100`. Đáp ứng FR-009 (1 gate quản lý tập trung) nhưng đây là gate mặc định chứ không phải gate tuỳ biến; spec nhắc "ngưỡng độ bao phủ, trùng lặp, lỗi blocker/critical mới" — không có điều kiện riêng cho blocker/critical (gom vào `new_violations`). Ngoài ra toàn project đang có coverage 78.8 % và trùng lặp 7.9 % (đều vượt ngưỡng nếu áp cho toàn bộ mã, nhưng gate chỉ đo phần mã mới).
- **`scripts/ci/sonar-begin.sh` hỏng trên Git Bash Windows nếu thiếu `MSYS_NO_PATHCONV=1`**: tham số `/k:ecommerce`, `/n:…`, `/d:…` bị MSYS đổi thành đường dẫn (`k:ecommerce`) → `Unrecognized command line argument: k:ecommerce` / "Expecting … project key". Jenkins chạy trên Linux nên không ảnh hưởng CI thật; chỉ ảnh hưởng ai chạy tay trên máy Windows. Kèm: file `.sh` trong working copy có CRLF (autocrlf) — vẫn chạy được bằng `sh` ở Git Bash.
- **Fail-closed khi SonarQube tắt (FR-008) — đúng thiết kế, đã đo**: tắt `sonarqube` rồi chạy `scripts/ci/sonar-begin.sh` (với `MSYS_NO_PATHCONV=1`) → `Unable to connect to server … /api/settings/values?component=unknown`, ném `HttpRequestException`, thoát mã **127** sau ~7 giây; chặng `sonarqube: begin analysis` sẽ vào nhánh `failure` → `checkFailed(ci/sonarqube-quality-gate)`. Chặng `sonarqube quality gate` có `timeout(15 phút)` + mọi trạng thái khác `OK` thành `error(...)`. (Mã thoát 127 thường nghĩa "command not found" nên dễ gây hiểu nhầm khi đọc log; nguyên nhân thật là ngoại lệ chưa xử lý.)
- **Jenkins cục bộ cần đăng nhập, mật khẩu không được tài liệu hoá**: `curl localhost:8080/` và `/job/ecommerce/api/json` đều `403` khi ẩn danh; volume có user `admin` và `nmhieuit` nhưng runbook `docs/github-jenkins-sonarqube-setup.md` đã bị xoá (xem mục 013) nên không ai biết cách vào — QA chỉ đọc được lịch sử build qua `docker exec … /var/jenkins_home/jobs/ecommerce/branches/*/builds/*`.
- **Tài liệu**: không có architecture/development/PO cho 012 (nguồn duy nhất còn lại là bản tóm tắt JSON), `technical-debt.md` không có mục riêng cho 012.
...
- **Lượt làm lại theo hướng Postman (2026-09-26) — nguyên nhân gốc rõ hơn và số liệu mới**: bật lại Jenkins/SonarQube cục bộ từ volume cũ (`docker compose -f docker-compose.ci.yml up -d --no-build`, cả 2 healthy). (1) `indexing.log` của job `ecommerce` (tự quét lúc Jenkins khởi động) → `"message": "Bad credentials", "status": "401"` … `Finished: FAILURE`: **Jenkins không quét được GitHub vì PAT `github-pat` trong credential đã vô hiệu** — nhất quán với log build cuối (2026-09-06, `fix-correl…` #1): `java.lang.IllegalArgumentException: The supplied credentials are invalid to login` ở `GitHubStatusNotificationStep` (`githubNotify` không đăng được check) — giải thích trực tiếp vì sao không còn check nào về GitHub từ đầu tháng 9, thay cho phỏng đoán "rule bị lưu rỗng" ở trên (2 nguyên nhân có thể cùng tồn tại, nhưng token hỏng đủ để CI không đăng check dù rule còn). (2) Folder Postman `13 - Chặn merge vào master (GitHub)` (API công khai): bước 01–02 xanh (repo public, `master` có cờ `protected: true`), bước 03 **đỏ** (`rules/branches/master` = `[]`, không rule `required_status_checks`), bước 04 **đỏ** (`commits/master/status` → `total_count: 0`, `state: pending`), bước 06 **đỏ** (commit merge của PR mới nhất — PR #52, 2026-09-26 — 0 check; PR #48 → #52 cùng ngày đều merge với 0 check), bước 07 **đỏ** (`branches/master/protection` với PAT → `401`). (3) **Số check bắt buộc là 7, không phải 5**: `scripts/ci/setup-branch-protection.sh` (dòng 84–90, sau spec 018) liệt kê 7 context (`ci/build`, `ci/unit-tests`, `ci/integration-tests`, `ci/contract-tests`, `ci/sonarqube-quality-gate`, `ci/secret-scan`, `ci/image-secret-scan`) và in "Expected: all seven ci/* checks", trong khi `architecture/013`, `quickstart.md` và doc QA cũ nói 5; chính comment trong script ghi việc chạy lại script để áp 2 context mới (tasks.md T036 của spec 018) "deliberately NOT performed" — chưa ai chạy, nên dù rule được dựng lại từ đầu thì trên GitHub vẫn có thể thiếu 2 context này. (4) `CI_FAST_ITERATION = 'true'` đang commit (mục 012): ngay cả khi thay PAT, 3 trong 7 check sẽ không bao giờ được đăng.
...
- **Lượt làm lại theo hướng Postman + công tắc `.env` (2026-09-26) — ma trận 2 công tắc trên stack thật**: folder Postman `14 - Xác thực tại gateway và từng service` (7 bước) ở trạng thái mặc định (`FEATURE_IDENTITY_SERVER_AUTH_CUTOVER=true`, `FEATURE_AUTHORIZATION_REQUIRE_API_SCOPE=true`) **9/9 xanh** cùng `00 → 01`: token có `sub`, `tenant_id = contoso`, `scope` chứa `ecommerce-api`; gateway không token / token bị sửa chữ ký `401`, token hợp lệ `200`; service gọi thẳng (bỏ qua gateway) tự chặn token giả/không token `401`, token thật `404` (đơn không tồn tại). Đổi công tắc trong `.env`, tạo lại 6 service, làm ấm 2 lượt rồi đo: **(false, false) — rollback**: gateway token hợp lệ `200`, **không token vẫn `401`**, token sửa `401` (do BFF/service tự xác thực, không phải gateway) → gạt `cutover=false` KHÔNG trả lại truy cập ẩn danh như Phase 1, chỉ bỏ bước xác thực ở gateway; **(false, true) — cảnh báo trong `.env.example`**: gateway trả `403` cho MỌI request kể cả token hợp lệ và không token (đúng lời cảnh báo, service gọi thẳng vẫn `401`/`404` như cũ); **(true, false)**: giống mặc định. Khớp phát hiện `403 forbidden_scope` phía trên và cho thấy đường "rollback khẩn cấp" của quickstart Scenario 7 chỉ dùng được khi client vẫn gửi token.
- **Request Postman `00 → 04 "Có token nhưng thiếu scope thì bị chặn (403)"` đỏ ở CẢ 4 trạng thái công tắc (đã đo, cần đối chiếu ở mục 015)**: token xin bằng `scope=openid profile` (không `ecommerce-api`) có `aud` chỉ là `http://identity-api:8080/resources` (không có `ecommerce-api`) nên `baskets-api` (gọi thẳng) và gateway đều trả **`401`**, không phải `403 forbidden_scope` như test và mô tả của request nêu (kể cả khi `scope=false`). Tức `403 forbidden_scope` chỉ xuất hiện với stub identity (`cutover=false` + `scope=true`), không đạt được bằng token thật từ identity — trái với chú thích của request ("mirror `AuthorizationPolicyTests.ARequestWithATokenMissingTheApiScopeClaim_IsForbidden`", test dùng token dựng tay có `aud` đúng nhưng thiếu `scope`). Đề xuất: sửa test Postman về `401` hoặc bổ sung cách phát token có `aud` nhưng thiếu `scope` để chứng minh nhánh `403`.
- **Lần đầu sau khi tạo lại container luôn chậm/lỗi dù đã `healthy`**: sau `up --force-recreate` 6 service, request đầu tới identity mất 12–14 giây, gateway `504` cho token hợp lệ (3.5–5 giây) ngay cả sau 2 lượt làm ấm ở trạng thái mặc định, rồi các lượt sau `200` (28–50 ms) — cùng hiện tượng nguội đã ghi ở mục 017/026 (lấy khoá ký từ identity, JIT). Ai làm thủ công theo `quickstart.md` nên gọi thử vài lần trước khi kết luận lỗi.
...
- **Lượt làm lại theo hướng Postman + công tắc `.env` (2026-09-26)**: folder Postman `15 - Phân quyền từ chối theo mặc định` (11 request) ở trạng thái mặc định **13/13 xanh**: với 5 service gọi thẳng cổng (products 5088, baskets 5188, orders 5041, parties 5204, bff 5301) health probe không token `200` (`AllowAnonymous`), route nghiệp vụ không token `401 {"error":"unauthorized"}`, gateway token hợp lệ `200`. Đổi `FEATURE_AUTHORIZATION_REQUIRE_API_SCOPE` / `FEATURE_IDENTITY_SERVER_AUTH_CUTOVER` (tạo lại 6 service, làm ấm 2 lượt): **(cutover=false, scope=true)** gateway trả `403 {"error":"forbidden_scope","message":"Authentication succeeded, but the token does not carry the required scope."}` cho CẢ token hợp lệ lẫn không token (stub Phase 1 không phát claim scope) — đây là lần duy nhất thấy `forbidden_scope` trên hệ thống thật; **(false, false)** token hợp lệ `200`, không token `401` (BFF tự xác thực); **(true, false)** `200`/`401` như mặc định. Route thử `/qa-probe-015-temp-route` thêm tạm vào `CatalogEndpoints.cs` không khai `RequireAuthorization()`/`AllowAnonymous()`: scanner `EveryMappedRoute_DeclaresAnAuthorizationDecision` đỏ nêu đích danh route + file; dựng lại `products-api` thì route đó không token `401`, có token `200` (FallbackPolicy che route quên khai báo); sau khi hoàn tác mọi đường dẫn lạ không token vẫn `401`.
- **`403 forbidden_scope` không đạt được bằng token thật từ `identity` (đã đo, liên quan mục 014)**: token xin bằng `scope=openid profile` (thiếu `ecommerce-api`) có `aud` không chứa `ecommerce-api` nên bị từ chối ở bước xác thực (`401`) trước khi tới chính sách `ApiScope`; nhánh `403 forbidden_scope` chỉ chạy với stub identity hoặc token dựng tay có `aud` đúng nhưng thiếu `scope` (đúng như các test tích hợp `AuthorizationPolicyTests`). Nghĩa là chính sách `ApiScope` hiện là lớp thứ 2 chưa từng lộ ra cho client thật, và request Postman `00 → 04` (kỳ vọng `403`) đỏ ở mọi trạng thái công tắc. Hai scanner `IConsumer<T>` vẫn đỏ 2/4 như đã ghi.
